...
BitSet createBitSet(Arena* arena, size_t cap) {
    ...
    uintptr_t* const words = (uintptr_t*)acalloc(arena, wordCap, sizeof *words);
...
BitSet bitSetClone(Arena* arena, BitSet const* bits) {
    ...
    uintptr_t* const words = (uintptr_t*)amalloc(arena, wordCap * sizeof *words);
...
void bitSetSet(Arena* arena, BitSet* bits, size_t n) {
    ...
    if (wordIdx >= bits->wordCap) { // Does not even fit allocation => grow:
        ...
        uintptr_t* const newWords = (uintptr_t*)amalloc(arena, newCap * sizeof *newWords);
