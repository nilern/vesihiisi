...
void X64SYSVJIT::naturalize(std::span<uint8_t const> bytecode) {
    ...
    for (auto it = bytecode.begin(); it != end;) {
        // TODO: Actually JIT-compile the bytecodes:
        ...
        case OP_TAILCALL:
        case OP_FFICALL:  {
            as_.mov(X64SYSVJIT::retReg, PrimopRes::INTERPRET);
            as_.ret();
            return;
        }; break;
        }
...
void X64SYSVJIT::jitMethod(Method& method) {
    ...
    if (!std::ranges::all_of(method.domain(),
        ...
        as_.and_(x86::r10, x86::Mem{X64SYSVJIT::rtReg, int32_t(calleeOffset)});
        ...
        as_.mov(x86::rax, x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->domainCheckingOffset())});
        as_.cmp(x86::rax, RT::DomainChecking::SKIP);
        as_.jne(doCheckDomain);
        //      domainChecking = RT::DomainChecking::CHECK;
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->domainCheckingOffset())},
        ...
        as_.mov(x86::rax, x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->entryRegcOffset())});
        ...
        if (method.hasVarArg.val()) {
            ...
            as_.mov(X64SYSVJIT::retReg, PrimopRes::CALL_BYTECODE);
            as_.ret();
        }
        ...
        // rt->method = method;
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->methodOffset())}, x86::r10);
        ...
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->codeOffset())}, x86::r11);
        ...
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(constsObjOffset)}, x86::r11);
        ...
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(constsSlotsOffset)}, x86::r11);
        // rt->pc = Method::entryPc();
        // `as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->pcOffset())}, Method::entryPc());` was
        // storing an incorrect value for some reason :(:
        as_.mov(x86::r11, Method::entryPc());
        as_.mov(x86::Mem{X64SYSVJIT::rtReg, int32_t(rt_->pcOffset())}, x86::r11);
