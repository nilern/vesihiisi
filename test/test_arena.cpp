...
static void test_amalloc(void) {
    Arena arena = newArena(defaultArenaBlockSize);
    uint8_t* const obj =(uint8_t*)amalloc(&arena, defaultArenaBlockSize / 2);
    assert(obj);
...
static void test_arealloc(void) {
    ...
    uint8_t* const obj = (uint8_t*)amalloc(&arena, oldSize);

    uint8_t* newObj = (uint8_t*)arealloc(&arena, obj, oldSize, defaultArenaBlockSize / 4 * 3);
    assert(newObj);
    assert(newObj != obj);
...
static void test_areallocHuge(void) {
    ...
    uint8_t* newObj = (uint8_t*)arealloc(&arena, obj, oldSize, defaultArenaBlockSize * 3);
    assert(newObj);
    assert(newObj != obj);
...
static void test_acalloc(void) {
    ...
    uint8_t* const obj = (uint8_t*)acalloc(&arena, count, size);
