...
(define main
  (fn args
    ...
    (run-benchmark
      "nboyer"
      1
      (fn (rewrites) (and rewrites (= rewrites 16445406)))
      (fn (n)
       (fn ()
         (setup-boyer)
...
(define setup-boyer (fn args #t));; assigned below
(define test-boyer (fn args #t));; assigned below
...
(let ()
  ...
  ;; A symbol-record is represented as a array! with two fields:
  ;; the symbol (for debugging) and
  ;; the list of lemmas associated with the symbol.

  (define put
    (fn (sym property value)
  ...
  (define put-lemmas!
    (fn (symbol-record lemmas)
      (array!-set! symbol-record 1 lemmas)))

  (define get-lemmas
    (fn (symbol-record)
      (array!-get symbol-record 1)))

  (define get-name
    (fn (symbol-record)
      (array!-get symbol-record 0)))
    ...
    (fn (alist term n)
      (let ((term
             (apply-subst
              ...
              (translate-term
               (induct ((term term (list 'or term '(f)))
                        (n n (- n 1)))
                       ((= n 0) term))))))
        (tautp term))))
  ...
  (define tautologyp
    (fn (x true-lst false-lst)
      (cond ((truep x true-lst)
            ...
            ((not (isa? <pair> x))
             #f)
            ((identical? (car x) if-constructor)
             (cond ((truep (cadr x)
                           true-lst)
                    (tautologyp (caddr x)
                                true-lst false-lst))
